...
class ModuleControl {
    ...
    async getModuleDetails(req, res) {
        console.log('Tentative de récupération des détails du module');
        try {
            const moduleDetails = await ModuleService.getModuleDetails();
            res.json(moduleDetails);
        } catch (error) {
            console.error('Erreur lors de la récupération des détails du module:', error.message, error.stack);
            res.status(500).json({ error: "Une erreur est survenue lors de la récupération des détails du module." });
        }
    }
}
